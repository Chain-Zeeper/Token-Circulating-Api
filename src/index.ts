...
import { circulating_supply, Token, total_supply } from './token';
...
app.get('/:ticker/circulating',async(req:Request,res:Response)=>{
  try{
    ...
    return res.send(ethers.formatEther(_circulating))
  }catch(err){
...
app.get('/:ticker/total-supply',async(req:Request,res:Response)=>{
  try{
    let {ticker} = req.params
    let token:Token | undefined = tokens[ticker as keyof typeof tokens] as unknown as Token
    if(!token) return res.status(400).json(
    ...
    let _total  = await total_supply(token);

    return res.status(200).json({"total_supply":ethers.formatEther(_total)})
  }catch(err){
    return res.status(400).json(
      {error:"400",
       message:`ERROR: ${err}`   
